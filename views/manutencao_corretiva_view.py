import flet as ft
from controllers.maquina_controller import listar_maquinas_corretiva, adicionar_maquina_corretiva, remover_maquina_corretiva

def manutencao_corretiva_view(page):
    def voltar(e):
        page.go("/manutencoes")
    ...
    def exibir_explicacao(e):
        ...
        dialog = ft.AlertDialog(
            title=ft.Text("O que é manutenção corretiva?"),
            content=ft.Text(
                "Manutenção corretiva é realizada após a ocorrência de falhas ou defeitos. "
                "Tem como objetivo reparar e restaurar o funcionamento do equipamento."
            ),
            actions=[
    ...
    def carregar_maquinas():
        tabela.rows.clear()
        for maq in listar_maquinas_corretiva():
            tabela.rows.append(
                ft.DataRow(cells=[
    ...
    def adicionar(e):
        if nome_input.value.strip() == "":
            return
        adicionar_maquina_corretiva(nome_input.value)
        nome_input.value = ""
        carregar_maquinas()

    def remover(id):
        remover_maquina_corretiva(id)
        carregar_maquinas()
    ...
    page.views.append(
        ft.View(
            ...
            controls=[
                ft.Row([ft.Text("Manutenção Corretiva", size=25, weight="bold")]),
                ft.Row([
